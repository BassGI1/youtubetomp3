...
from os import rename, environ, path
...
if str(input("Am I downloading a single video or a playlist (public or unlisted)? ")) in videoArray:
    ...
    try:
        ...
        Desktop = environ["HOMEPATH"] + '\Desktop\\' + str(input("Where should I place the downloaded file? "))
        download = video.download(output_path=Desktop)
        name, extension = path.splitext(download)
        rename(download, f'{name}.mp3')
    ...
    try:
        ...
        Desktop = environ["HOMEPATH"] + '\Desktop\\' + str(input("Where should I place the downloaded files? "))
